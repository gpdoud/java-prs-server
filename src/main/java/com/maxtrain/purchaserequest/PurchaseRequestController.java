...
public class PurchaseRequestController {
	...
	private void SetForeignKeyInstances(PurchaseRequest purchaseRequest) {
		...
		for(PurchaseRequestLineitem line : lines) {
			int prliid = line.getId();
//			line.setPurchaseRequest(purchaseRequestRepository.findOne(line.getPurchaseRequestId()));
			line.setProduct(productRepository.findOne(line.getProductId()));
		}
	...
	public @ResponseBody Iterable<PurchaseRequest> List() {
		...
		for(PurchaseRequest purchaseRequest : purchaseRequests) {
			SetForeignKeyInstances(purchaseRequest);
		}
